...
cd ..
colcon build --packages-select ls2n_drone_px4_autopilot --symlink-install
export GZ_VERSION=harmonic #ALWAYS
```
...
colcon build --packages-select ls2n_drone_command_center ls2n_drone_qualisys --symlink-install
...
git reset --hard origin/release/1.15
cd ..
cd ..
colcon build --symlink-install --packages-select px4_msgs ls2n_drone_bridge

```

- From ls2n_drone_controllers
```bash
cd src
...
source install/setup.bash
cd .. # in the folder above the workspace
...
# Install it globally so the LS2N launch script can find it from anywhere
sudo make install
...
- Others
```bash
sudo apt install ros-jazzy-nav2-common ros-jazzy-navigation2
...
colcon build --symlink-install --packages-select px4_msgs ls2n_drone_px4_autopilot ls2n_drone_interfaces ls2n_drone_controllers ls2n_drone_bridge
colcon build --symlink-install --packages-select ls2n_drone_armada
source install/setup.bash
...
export XRCE_MAX_PAYLOAD_SIZE=512
