...
class AnnotatedDatabaseBuilder:
    def __init__(self, root, img_path, GRID_MAPPING):
        self.root = root
    ...
    def load_active_option(self):
        ...
        if self.current_idx < len(self.options_to_map):
            ...
            # Clear canvas drawing elements
            self.image_canvas.delete("all")
            self.draggable_labels.clear()

            # Render the corresponding cropped sub-image
            if os.path.exists(self.img_path):
                orig_img = Image.open(self.img_path)
                crop_box = GRID_MAPPING[target]
                cropped_snippet = orig_img.crop(crop_box)
                
                # Resize snippet so it fits comfortably inside our preview viewport window height
            ...
            self.export_database_to_file()

    def build_node_rows(self):
        """Builds numeric parameter table rows and spawns draggable layout markers on image canvas."""
        for widget in self.scrollable_frame.winfo_children():
            widget.destroy()
        
        # Strip older labels off the canvas block tracking
        ...
        # Build interactive entry structures 
        for i in range(num_points):
            ...
            ent_z = ttk.Entry(self.scrollable_frame, width=7); ent_z.insert(0, "-0.1"); ent_z.grid(row=row, column=3, padx=2)
            ...
            # --- SPAWN DRAGGABLE TARGET ANNOTATION ON THE IMAGE ---
            # Stagger labels at top canvas coordinate spaces initially
            spawn_x = 30 + (i * 35)
            spawn_y = 35
            
            # Draw visual highlight dot backing circle for high readability matching letters
            ...
            # Map structural group identifier
    ...
    # --- DRAG & DROP ANNOTATION EVENT UTILITIES ---
    ...
    def on_label_drag(self, event):
        if self.selected_label_id is not None:
            ...
            # Resolve paired entities (text or oval container companion)
            ...
            # Shift objects positions interactively on canvas element tracker
    ...
    def save_current_layout(self):
        """Compiles tabular structures and saves graphic node positions into the database map."""
        ...
        # Harvest canvas drawing locations to save visual marker points map
        visual_positions = {}
        for item_id, details in self.draggable_labels.items():
            if "circle" in details:  # Evaluate using text center references points
        ...
        try:
            for item in self.node_inputs:
                ...
                nodes_dictionary[let] = {
                    "coords": [float(item["x"].get()), float(item["y"].get()), float(item["z"].get())],
        ...
        self.database[current_target] = nodes_dictionary
        self.current_idx += 1
        self.load_active_option()

    def export_database_to_file(self):
        filename = "configuration_database.py"
        with open(filename, "w") as f:
            f.write("# Programmatically Generated Configuration Database with Annotation Positions\n")
            f.write("GEOMETRIC_DATABASE = ")
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    app = AnnotatedDatabaseBuilder(root, IMAGE_PATH, GRID_MAPPING)
    root.mainloop()